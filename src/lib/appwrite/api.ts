...
export async function signOutAccount(){
  try {
    const session = await account.deleteSession('current'); //this will delete current session
...
export async function createPost(post: INewPost) {
  try {
    ...
    // Save the post to the databases
    ...
    if (!newPost) {
      await deleteFile(uploadedFile.$id); //if post is not created then delete file
      throw Error;
    }
